...
def make_prediction(model, file_name='test_backpacks.jpg', is_custom = False):
    ...
    if is_custom: # image prediction
        ...
        print(file_name)
        print(predictions)
        print("Evens (left) are backpacks / Odds (right) are mobile phones")
    else:
...
def create_training_set(total = 50, augmented = False):
    file_name = 'Real_Classification.v1-raw\\train\\mix'+chr(92)+"1.jpg"
    img = Image.open(file_name)
    img = cv2.resize(np.array(img), (224, 224))  # resize image to match model's expected sizing
    img = img.reshape(1, 224, 224, 3)  # return the image with shaping that TF wants.
    ...
    if augmented:
        ...
        flipped_vertical = tf.image.flip_up_down(img)

        numpydata_flip = asarray(flipped)
        numpydata_saturated = asarray(saturated)
        numpydata_flip_vertical = asarray(flipped_vertical)

        numpydata_original = np.concatenate([numpydata_original, numpydata_flip], axis=0)
        numpydata_original = np.concatenate([numpydata_original, numpydata_saturated], axis=0)
        numpydata_original = np.concatenate([numpydata_original, numpydata_flip_vertical], axis=0)

    #print(numpydata_original)

    for x in range(2, total+1):
        file_name = 'Real_Classification.v1-raw/train/mix/' + str(x) + ".jpg"
...
file_path = 'Real_Classification.v1-raw/train/mix/'
pic_name = '2'
is_custom = False

make_prediction(model, file_path+pic_name+".jpg", is_custom)
make_prediction(model, file_path+"test_backpacks"+".jpg", is_custom)
make_prediction(model, file_path+"test_mobilePhones"+".jpg", is_custom)
